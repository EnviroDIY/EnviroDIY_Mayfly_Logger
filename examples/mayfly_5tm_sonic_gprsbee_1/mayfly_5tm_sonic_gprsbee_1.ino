...
char tmaddress = '1';      //for one sensor on channel '1'
float CTDtempC, CTDcond, soiltemp, soilEa, soilVWC;
int range_mm;
...
#define DATA_HEADER "DateTime_EST,TZ-Offset,Loggertime,BoardTemp,Battery_V,soiltemp_DegC,soilEa,soilVWC,SonarRange_mm"
...
void setup()
{
  ...
  pinMode(GPRSBEE_PWRPIN, OUTPUT);    // Bee socket DTR pin
  digitalWrite(GPRSBEE_PWRPIN, LOW);   // on GPRSbee v6, setting this high turns on the GPRSbee.  leave it high to keep GPRSbee on.
...
//  ads.begin();       //begin adafruit ADS1015
  ...
  //Make first call
  Serial.println("Power On, running: SL0xx_mayfly_5tm_sonic_gprsbee_1.ino");
  //showTime(getNow());
...
void loop()
{
  ...
  if(currentminute % testminute == 0)
     {   //Serial.println("Multiple of x!!!   Initiating sensor reading and logging data to SDcard....");
          ...
          digitalWrite(GPRSBEE_PWRPIN, HIGH);
          delay(1000);
          ...
          digitalWrite(GPRSBEE_PWRPIN, LOW);
...
void tmMeasurement(char i){        //5TM soil moisture sensor
  String command = "";
  soilEa = 0.0;
  soiltemp = 0.0;
  soilVWC = 0.0;
  ...
  delay(500);
     ...
     if(mySDI12.available() > 0){
        ...
        soilEa = mySDI12.parseFloat();
        soiltemp = mySDI12.parseFloat();

      soilVWC = (4.3e-6*(soilEa*soilEa*soilEa)) - (5.5e-4*(soilEa*soilEa)) + (2.92e-2 * soilEa) - 5.3e-2 ;     //the TOPP equation used to calculate VWC

      dataRec += ",";
      addFloatToString(dataRec, soiltemp, 3, 1);
      dataRec += ",";
      addFloatToString(dataRec, soilEa, 3, 1);
      dataRec += ",";
      addFloatToString(dataRec, soilVWC, 3, 2);
     }
      mySDI12.flush();

}  //end of tmMeasurement
...
{
    targetURL = "";
    targetURL = "http://swrcsensors.dreamhosters.com/xxxxxxxx.php?";
    targetURL += "LoggerID=SL0xx&Loggertime=";
    targetURL += currentepochtime;
    targetURL += "&soiltemp=";
    addFloatToString(targetURL, soiltemp, 3, 1);    //float
    targetURL += "&soilEa=";
    addFloatToString(targetURL, soilEa, 3, 1);    //float
    targetURL += "&soilVWC=";
    addFloatToString(targetURL, soilVWC, 3, 2);    //float
